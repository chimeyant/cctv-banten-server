...
class CctvService {
  ...
  async lists(){
    const records = await Cctv.query().preload('area').orderBy("area_uuid",'asc')
    ...
    records.forEach(element => {
      ...
      row['url']= element.url
      row['public_access']= element.publicAccess ? {color:'green', text:'AKTIF'}: {color:'red',text:"TIDAK AKTIF"}
      datas.push(row)
    });
  ...
  async update(payload:CctvInterface, id:string){
    try {
      const model = await Cctv.findBy("uuid",id)
      model?.merge({areaUuid: payload.area_uuid, name:payload.name, description:payload.description, contactPerson: payload.contact_person, phone:payload.phone, url:payload.url ,address: payload.address, lat: payload.lat, lng: payload.lng, publicAccess: payload.public_access, foto: payload.foto, status: payload.status})
